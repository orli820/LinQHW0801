...
namespace LinqLabs.作業
{
    public partial class Frm作業_4 : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            var q = nw.Orders.AsEnumerable().GroupBy(y => y.OrderDate.Value.Year).
                Select(p => new { key = p.Key, total = (float)(p.Sum(o => o.Order_Details.Sum(i => i.UnitPrice * i.Quantity * (decimal)(1 - i.Discount)))) });
            var q1 = nw.Orders.AsEnumerable().GroupBy(y => y.OrderDate.Value.Year).
                Select(p => new { key1 = p.Key + 1, total1 = p.Sum(o => (float)(o.Order_Details.Sum(i => i.UnitPrice * i.Quantity * (decimal)(1 - i.Discount)))) });
            var q2 = from k in q
                     join j in q1 on k.key equals j.key1
                     select new {年營收成長率 =$"{((( k.total -j.total1) / j.total1) * 100):n2}" ,Year=k.key };            
            dataGridView1.DataSource = q2.ToList();
